...
class WindowPrin(QMainWindow):
    def __init__(self,parent=None):
        ...
        self.PrinWin.pushButton_12.clicked.connect(lambda: self.EliminarVen(self.PrinWin.pushButton_12))
        self.PrinWin.pushButton_13.clicked.connect(lambda: self.Ticket.exec())
        self.PrinWin.lineEdit.textChanged.connect(lambda: self.buscarProducto())
        self.PrinWin.tabWidget.currentChanged.connect(self.onChange)
        self.PrinWin.tabWidget.tabBarClicked.connect(lambda: self.ProductosComprados())
        self.PrinWin.Enviar.clicked.connect(lambda: self.Visualizacion())
        self.PrinWin.MenuCasa.clicked.connect(lambda:self.menu())
        self.Sliderconfig()
        #---------------------------------------------------------------------------------------
        self.Ticket.Tiketwind.pushButton_4.clicked.connect(lambda: self.eliminarCompras())
        self.user.prin.pushButton_10.clicked.connect(lambda: self.eliminarCompras())
        self.user.prin.pushButton_10.clicked.connect(lambda: self.Ticket.elimiarCompras())
        self.Ticket.Tiketwind.pushButton_2.clicked.connect(lambda: self.eliminarCompras())
        self.Ticket.Tiketwind.pushButton_5.clicked.connect(lambda: self.AjusteDeContador())
        self.botonesDeProduct(self.PrinWin.Casa,None)#se manda a llamar el metodo para crear la lista de los productos
        #----------------------------------------------------------
        with open("CSS/stylesMenuPrinw.css","r") as f:
            self.setStyleSheet(f.read())#abre el archivo de estilos para el menu principal
    ...
    def Maxi(self):
        if self.isMaximized():
            self.iconba = QIcon()
            self.iconba.addFile(u":/ICONS/maximizar.png", QSize(), QIcon.Normal, QIcon.Off)
            self.PrinWin.pushButton_3.setIcon(self.iconba)
            self.PrinWin.pushButton_3.setIconSize(QSize(30, 30))
            self.showNormal()
        else:
            self.iconba2 = QIcon()
            self.iconba2.addFile(u":/ICONS/cuadricula.png", QSize(), QIcon.Normal, QIcon.Off)
    ...
    def AjusteDeContador(self):
        ...
        with open("CSS/stylesBoton.css","r") as f:
            self.PrinWin.pushButton_13.setStyleSheet(f.read())#estilos para el boton 
        self.PrinWin.pushButton_13.setText(str(self.contadorDeCompras))#visualizción de compras
    ...
    def buscarProducto(self):
        ...
        if self.PrinWin.radioButton_2.isChecked():
            ...
            self.botonesDeProduct(self.PrinWin.frame_8,BuscarProducto(Busqueda,"Producto"))
        elif self.PrinWin.radioButton_5.isChecked():
            self.botonesDeProduct(self.PrinWin.frame_8,BuscarProducto(Busqueda,"Tipo"))
        elif self.PrinWin.radioButton_3.isChecked():
            self.botonesDeProduct(self.PrinWin.frame_8,BuscarProducto(Busqueda,"Precio"))
        elif self.PrinWin.radioButton_4.isChecked():
            self.botonesDeProduct(self.PrinWin.frame_8,BuscarProducto(Busqueda,"Cantidad"))
    ...
    def botonesDeProduct(self,widget,actuali):
        ...
        if Data!=[]:#si la lista de productos no esta vacia
            ...
            while (len(Data)>len(self.ListaDeWidgets)):                
                ...
                with open("CSS/stylesBoton.css","r") as f:
                    self.ListaDeWidgets[self.i].setStyleSheet(f.read())#estilos para cada boton
                if widget != self.PrinWin.Casa:
                    widget.setMinimumSize(QSize(1300,self.contadorY+415))#tamaño del widget
                    self.ListaDeWidgets[self.i].setGeometry(QRect((315*self.contadorX)+90,self.contadorY+75,200,300))#posicion
                    self.ListaDeButons[self.i].setGeometry(QRect((315*self.contadorX)+115,self.contadorY+320,150,35))
                elif widget == self.PrinWin.Casa:
                    widget.setMinimumSize(QSize(1300,self.contadorY+315))#tamaño del widget
                    self.ListaDeWidgets[self.i].setGeometry(QRect((315*self.contadorX)+60,self.contadorY+15,200,300))#posicion
                    self.ListaDeButons[self.i].setGeometry(QRect((315*self.contadorX)+85,self.contadorY+260,150,35))
                #conectar cada uno de los metos con la posicion de cada boton
                if widget== self.PrinWin.frame_8:#si es cliente
                    self.ListaDeWidgets[self.i].clicked.connect(partial(self.CompraProduc,Data[self.i]))
                    self.ListaDeButons[self.i].clicked.connect(partial(self.CompraProduc,Data[self.i]))
                    self.ListaDeButons[self.i].setText("Comprar")   
                elif widget == self.PrinWin.widget_6:#si es vendedor
                    self.ListaDeWidgets[self.i].clicked.connect(partial(self.ProductoVen,Data[self.i]))
                    self.ListaDeButons[self.i].clicked.connect(partial(self.ProductoVen,Data[self.i]))
                    self.ListaDeButons[self.i].setText("Seleccionar")
                else:
                    self.ListaDeWidgets[self.i].clicked.connect(partial(self.ComentarioProduc,Data[self.i]))
                    self.ListaDeButons[self.i].clicked.connect(partial(self.ComentarioProduc,Data[self.i]))
                ...
                self.WidgetInfoProduc(self.ListaDeWidgets[self.i],Data[self.i])#se agregan caracteristicas de la vista
                if self.contadorX==3:#contador con cada salto
                    self.contadorY += 315
                    self.contadorX = 0
                else:self.contadorX += 1
                self.i+=1
        else:
    ...
    def EliminarVen(self,info):
        ...
        if self.PrinWin.pushButton_12==info and self.ID_venta !=None:
            ...
            self.botonesDeProduct(self.PrinWin.widget_6,None)#Actualiza lista de Productos
            #de lo contrario la informacion se aguarda
        else:self.ID_venta=info
    ...
    def CompraProduc(self,data):
        ...
        self.PrinWin.pushButton_13.setText(str(self.contadorDeCompras))#visualizción de compras
        if self.dato != 0:
            self.Ticket.infoCompra((data[2],data[4]),self.contadorDeCompras,'agregar',[self.dato[1],data[2],data[3],data[4]])
        else:
            self.Ticket.infoCompra((data[2],data[4]),self.contadorDeCompras,'agregar',0)
    ...
    def WidgetInfoProduc(self,widget,info):
        ...
        self.verticalSpacer = QSpacerItem(22, 41, QSizePolicy.Minimum, QSizePolicy.Preferred)
    ...
    def guardarVenta(self):
        if self.dato[3] == "Vendedor":
            ...
            if Nombre !='' and Tipo!='' and Precio!='' and Cantidad!='':
                insertarVenta((self.dato[1],Nombre,Tipo,int(Precio),int(Cantidad)))
                self.PrinWin.lineEdit_4.clear()
                self.PrinWin.lineEdit_6.clear()
                self.PrinWin.lineEdit_7.clear()
                self.PrinWin.lineEdit_8.clear()
        ...
        if self.dato !=0:
            ...
            for (index_col, col) in enumerate(compra):
                for (index_cell, cell) in enumerate(col):
                    self.PrinWin.tableWidget.setItem(index_col,index_cell, QTableWidgetItem(str(cell)))
        else:
            compra=[]
            self.PrinWin.tableWidget.setRowCount(len(compra))
            for (index_col, col) in enumerate(compra):
    ...
    def menu(self):
        ...
        if self.PrinWin.widget_7.width()==0:
            ...
            TamaxWidget_7=(340,0)                
        self.user.animationWidget(self.PrinWin.widget_7,200,TamaxWidget_7[0],TamaxWidget_7[1],False,False,'maximumWidth')
        
    def Sliderconfig(self):
        self.PrinWin.verticalSlider.sliderReleased.connect(lambda:self.valueSlider(self.PrinWin.verticalSlider))
        self.PrinWin.verticalSlider_2.sliderReleased.connect(lambda:self.valueSlider(self.PrinWin.verticalSlider_2))
        self.PrinWin.verticalSlider_3.sliderReleased.connect(lambda:self.valueSlider(self.PrinWin.verticalSlider_3))
        self.PrinWin.verticalSlider_4.sliderReleased.connect(lambda:self.valueSlider(self.PrinWin.verticalSlider_4))
    ...
    def valueSlider(self,widget):
        ...
        self.PrinWin.frame.setStyleSheet(f"background-color:rgba({self.configcolor[0]},{self.configcolor[1]},{self.configcolor[2]},{self.configcolor[3]});")
        self.PrinWin.ColorRGBA.setText(f"    R={self.configcolor[0]}       G={self.configcolor[1]}      B={self.configcolor[2]}      A={self.configcolor[3]}")
        ...
        for i in BusquedaMens: 
            for j in range(len(i)):
                if j !=0 and j%3==0:
                    ...
                    aux=" ->"+user+"\n"
                    Mensaje=i[j-1]
                    self.ListaDeWidgetsMensaje.insert(self.contadorDeMensjes,QWidget(self.PrinWin.MensajesDeTexto)) 
                    self.ListaDeWidgetsMensaje[self.contadorDeMensjes].setVisible(True)
                    TamTaxto=0
                    contador=0
                    for i in Mensaje:
                        aux+=i
                    ...
                    self.PrinWin.MensajesDeTexto.setMinimumSize(QSize(320,tamano+20))
                    aux=""
